...
(() => {
    ...
    function render(){
        ...
        //戻したいポイント == 割り算の右辺 にすれば 戻したいポイントをコントロールできる
        //ここではウインドウの右端で戻したいので window.innerWidth を指定している
        //さらに円の右端からウインドウに入って左端で消えて行かせたいので、ウインドウ幅に円の直径を足す
        //だたこのままでは画面の左端を増やしただけになってしまうので、円の中心から見えてしまう。
        //そこで次の行
        let x = nowTime % (window.innerWidth + CIRCLE_RADIUS * 2);

        // X 座標から円の半径を引く
        // 円の半径を引くことで始まりのポイントが 円の右端から出てくるようになる
        x = x - CIRCLE_RADIUS;
